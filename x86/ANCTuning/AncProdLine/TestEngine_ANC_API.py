...
def getSinkID(engine, handle, device, instance, channel):
    '''
    get ID to sink (ADC)
    engine      - handle to Testengine.dll
    handle      - handle to 8675 device
    ...
    instance    - hardware instance, can be 0,1,2
    channel     - 0 = left channel, 1 = right channel
    Returns     - sinkID stream ID
    Example: sinkID = getSinkID(engine, handle, DEVICE, ADC_INSTANCE, CHANNEL_A)
    '''
    if device == ANALOG_MIC:
        print('Using analog mic')
    elif device == DIGITAL_MIC:
    ...
    success, sinkID = engine.teAudioGetSink(handle, ct.c_uint16(device), ct.c_uint16(instance), ct.c_uint16(channel))
    if not(success):
        raise ValueError('Sink ID failed')
    return sinkID
...
def smLPFEnable(engine, handle, anc_mic_path, enable, instance_mask):
    if (anc_mic_path.instance == None):
        ...
        smLPFEnableOk = audioConfigure(engine, handle, streamID, anc_mic_path.smLPF_enable, enable | instance_mask)
        if smLPFEnableOk == 1:
            if enable == 1:
...
def smLPFShift(engine, handle, anc_mic_path, shift, instance_mask):
    if (anc_mic_path.instance == None):
        print(anc_mic_path.name,' instance does not seem to be configured.')
        smLPFShiftOk = -1
    else:
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        smLPFShiftOk = audioConfigure(engine, handle, streamID, anc_mic_path.smLPF_shift, shift | instance_mask)
        if smLPFShiftOk == 1:
...
def dcFilter_Enable(engine, handle, anc_mic_path, enable, instance_mask):
    if (anc_mic_path.instance == None):
        if (enable):
            print(anc_mic_path.name, ' instance does not seem to be configured.')
        dcFilter_EnableOk = -1
        ...
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        dcFilter_EnableOk = audioConfigure(engine, handle, streamID, anc_mic_path.dcfilter_enable, enable | instance_mask)
...
def outmix_Enable(engine, handle, stream_control, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        if (enable):
            print(stream_control.key, ' instance does not seem to be configured.')
        outmix_EnableOk = -1
    else:
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.outmix
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.outmix if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE
        outmix_EnableOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if outmix_EnableOk == 1:
            if enable == 1:
                print('ANC Outmix enabled')
                setParam('OFFSET_ANC_OUTMIX_ENABLE_L',1)
            else:
                print('ANC Outmix disabled')
                setParam('OFFSET_ANC_OUTMIX_ENABLE_L',0)
        else:
            raise ValueError('Error when attempting to enable Outmix')
    return outmix_EnableOk

def anc1usesAnc0_Enable(engine, handle, stream_control, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        if (enable):
            print(stream_control.key, ' instance does not seem to be configured.')
        anc1usesAnc0_EnableOk = -1
    else:
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.anc1_uses_anc0_enable
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.anc1_uses_anc0_enable if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE
        anc1usesAnc0_EnableOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if anc1usesAnc0_EnableOk == 1:
            if enable == 1:
                print('anc1usesAnc0 RX PCM enabled')
                setParam('OFFSET_ANC1_USES_ANC0_RX_PCM_INPUT',1)
            else:
                print('anc1usesAnc0 RX PCM disabled')
                setParam('OFFSET_ANC1_USES_ANC0_RX_PCM_INPUT',0)
        else:
            raise ValueError('Error when attempting to enable anc1usesAnc0')
    return anc1usesAnc0_EnableOk

def ffgain_zcd_Enable(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        if (enable):
            print(stream_control.key, ' instance does not seem to be configured.')
        ffgain_zcd_EnableOk = -1
    else:
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.ffgain_zcd_enable
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.ffgain_zcd_enable if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        ffgain_zcd_EnableOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if ffgain_zcd_EnableOk is not 1:
            raise ValueError('Error when attempting to enable FFGain ZCD')
    return ffgain_zcd_EnableOk

def ffle_Enable(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        ...
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.ffle_enable
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.ffle_enable if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        ffle_EnableOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if ffle_EnableOk == 1:
            if enable == 1:
                print('ANC FFLE enabled')
                if stream_control.name == 'Anc0Control' or stream_control.name == 'Anc0Control1':
                    setParam('OFFSET_FF_FLEX_ENABLE_L',1)
                elif stream_control.name == 'Anc1Control' or stream_control.name == 'Anc1Control1':
                    setParam('OFFSET_FF_FLEX_ENABLE_R',1)
            else:
                print('ANC FFLE disabled')
                if stream_control.name == 'Anc0Control' or stream_control.name == 'Anc0Control1':
                    setParam('OFFSET_FF_FLEX_ENABLE_L',0)
                elif stream_control.name == 'Anc1Control' or stream_control.name == 'Anc1Control1':
                    setParam('OFFSET_FF_FLEX_ENABLE_R',0)
        else:
            raise ValueError('Error when attempting to enable FFLE')
    return ffle_EnableOk

def self_rxmix_Enable(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        if (enable):
            print(stream_control.key, ' instance does not seem to be configured.')
        self_rxmix_EnableOk = -1
    else:
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.self_rxmix_enable
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.self_rxmix_enable if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        self_rxmix_EnableOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if self_rxmix_EnableOk == 1:
            if enable == 1:
                print('ANC Self RxMix enabled')
                if stream_control.name == 'Anc0Control1':
                    setParam('OFFSET_SELF_RXMIX_ENABLE_L',1)
                elif stream_control.name == 'Anc1Control1':
                    setParam('OFFSET_SELF_RXMIX_ENABLE_R',1)
            else:
                print('ANC Self RxMix disabled')
                if stream_control.name == 'Anc0Control1':
                    setParam('OFFSET_SELF_RXMIX_ENABLE_L',0)
                elif stream_control.name == 'Anc1Control1':
                    setParam('OFFSET_SELF_RXMIX_ENABLE_R',0)
        else:
            raise ValueError('Error when attempting to enable Self RxMix')
    return self_rxmix_EnableOk

def cross_rxmix_Enable(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        if (enable):
            print(stream_control.key, ' instance does not seem to be configured.')
        cross_rxmix_EnableOk = -1
    else:
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.cross_rxmix_enable
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.cross_rxmix_enable if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        cross_rxmix_EnableOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if cross_rxmix_EnableOk == 1:
            if enable == 1:
                print('ANC Cross RxMix enabled')
                if stream_control.name == 'Anc0Control1':
                    setParam('OFFSET_CROSS_RXMIX_ENABLE_L',1)
                elif stream_control.name == 'Anc1Control1':
                    setParam('OFFSET_CROSS_RXMIX_ENABLE_R',1)
            else:
                print('ANC Cross RxMix disabled')
                if stream_control.name == 'Anc0Control1':
                    setParam('OFFSET_CROSS_RXMIX_ENABLE_L',0)
                elif stream_control.name == 'Anc1Control1':
                    setParam('OFFSET_CROSS_RXMIX_ENABLE_R',0)
        else:
            raise ValueError('Error when attempting to enable Cross RxMix')
    return cross_rxmix_EnableOk

def x0p5_a_select(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        ...
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.x0p5_a_select
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.x0p5_a_select if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        x0p5_a_selectOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if x0p5_a_selectOk is not 1:
            raise ValueError('Error when attempting to enable X0P5_A_SEL')
    return x0p5_a_selectOk

def x0p5_b_select(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        ...
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.x0p5_b_select
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.x0p5_b_select if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        x0p5_b_selectOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if x0p5_b_selectOk is not 1:
            raise ValueError('Error when attempting to enable X0P5_B_SEL')
    return x0p5_b_selectOk

def x2_a_select(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        ...
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.x2_a_select
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.x2_a_select if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        x2_a_selectOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if x2_a_selectOk == 1:
            if enable == 1:
                print('ANC X2_A_SEL enabled')
                if stream_control.name == 'Anc0Control':
                    setParam('OFFSET_DMIC_X2_FF_A_ENABLE_L',1)
                elif stream_control.name == 'Anc1Control':
                    setParam('OFFSET_DMIC_X2_FF_A_ENABLE_R',1)
            else:
                print('ANC X2_A_SEL disabled')
                if stream_control.name == 'Anc0Control':
                    setParam('OFFSET_DMIC_X2_FF_A_ENABLE_L',0)
                elif stream_control.name == 'Anc1Control':
                    setParam('OFFSET_DMIC_X2_FF_A_ENABLE_R',0)
        else:
            raise ValueError('Error when attempting to enable X2_A_SEL')
    return x2_a_selectOk

def x2_b_select(engine, handle, stream_control, anc_inst, enable):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        ...
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.x2_b_select
        CONTROL_ENABLE  = CONTROL_ENABLE | (stream_control.x2_b_select if enable == 1 else 0)
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        x2_b_selectOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if x2_b_selectOk == 1:
            if enable == 1:
                print('ANC X2_B_SEL enabled')
                if stream_control.name == 'Anc0Control':
                    setParam('OFFSET_DMIC_X2_FF_B_ENABLE_L',1)
                elif stream_control.name == 'Anc1Control':
                    setParam('OFFSET_DMIC_X2_FF_B_ENABLE_R',1)
            else:
                print('ANC X2_B_SEL disabled')
                if stream_control.name == 'Anc0Control':
                    setParam('OFFSET_DMIC_X2_FF_B_ENABLE_L',0)
                elif stream_control.name == 'Anc1Control':
                    setParam('OFFSET_DMIC_X2_FF_B_ENABLE_R',0)
        else:
            raise ValueError('Error when attempting to enable X2_B_SEL')
    return x2_b_selectOk

def fbmon_select(engine, handle, stream_control, anc_inst, select):
    CONTROL_SELECT  = 0
    CONTROL_ENABLE  = 0
    CONTROL_MASK    = 0
    if (stream_control.instance == None):
        ...
        streamID = getSourceID(engine, handle, stream_control.device, stream_control.instance, stream_control.channel)
        CONTROL_SELECT = CONTROL_SELECT | stream_control.fbmon_select
        CONTROL_ENABLE  = CONTROL_ENABLE | select
        CONTROL_MASK  = (CONTROL_SELECT << CONTROL_SEL_POS) | CONTROL_ENABLE

        audioConfigure(engine, handle, streamID, STREAM_ANC_INSTANCE, anc_inst)
        fbmon_selectOk = audioConfigure(engine, handle, streamID, stream_control.key, CONTROL_MASK)
        if fbmon_selectOk == 1:
            if BOARD == ANC_V1P9:
                if select == 1:
                    print('FB path IIR input goes to Monitor output')
                    if stream_control.name == 'Anc0Control':
                        setParam('OFFSET_FB_MON_L',1)
                    elif stream_control.name == 'Anc1Control':
                        setParam('OFFSET_FB_MON_R',1)
                else:
                    print('FFa path IIR input goes to Monitor output')
                    if stream_control.name == 'Anc0Control':
                        setParam('OFFSET_FB_MON_L',0)
                    elif stream_control.name == 'Anc1Control':
                        setParam('OFFSET_FB_MON_R',0)
            elif BOARD == ANC_V2P0:
                if select == 2:
                    print('ANC output goes to Monitor output')
                    if stream_control.name == 'Anc0Control':
                        setParam('OFFSET_FB_MON_L',2)
                    elif stream_control.name == 'Anc1Control':
                        setParam('OFFSET_FB_MON_R',2)
                elif select == 1:
                    print('FB path IIR input goes to Monitor output')
                    if stream_control.name == 'Anc0Control':
                        setParam('OFFSET_FB_MON_L',1)
            ...
            raise ValueError('Error when attempting to select FBMON_SEL')
    return fbmon_selectOk

def dcFilter_Shift(engine, handle, anc_mic_path, shift, instance_mask):
    if (anc_mic_path.instance == None):
        print(anc_mic_path.name, ' instance does not seem to be configured.')
        dcFilter_ShiftOk = -1
    else:
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        dcFilter_ShiftOk = audioConfigure(engine, handle, streamID, anc_mic_path.dcfilter_shift, shift | instance_mask)
...
def AncGainConvertFromdB(gainDB):
    ...
    #gain is negative
        ...
        # check for lowest shift value of -24 or -48
        if (gainDB < MIN_GAIN_DB):
            gainDB = MIN_GAIN_DB
            shift = MIN_GAIN_SHIFT # set shift to lowest value of -48dB or -24dB
            gain = MIN_GAIN
        else:
            # find nearest shift value
            a = gainDB / 6
...
def AncGainConvertToDb(shift, gain):
    ...
    shift - LPF gain shift value (-4 to 6) and (-8 to 6) for Mora 2.0 
    ...
    if(BOARD == ANC_V2P0):
        shift_gains = [0,6,12,18,24,30,36,0,-48,-42,-36,-30,-24,-18,-12,-6]
...
def ConfigureStreams(engine, handle, streamID, gain, shift, gainDB, instance_mask):
    ''' Get Source ID then write gain and shift values to
        specified filter '''
    gainval = AncGainConvertFromdB(gainDB)
    gainOk = audioConfigure(engine, handle, streamID, gain, gainval[1] | instance_mask)
    if not(gainOk):
        raise ValueError('Error configuring fine gain, please check that selected audio path is active')
    shiftOk = audioConfigure(engine, handle, streamID, shift, gainval[0] | instance_mask)
    if not(shiftOk):
...
def AncSetGain(engine, handle, gainDB, instance_mask, anc_mic_path):
    start_time = timeit.default_timer()
    ...
    if anc_mic_path.instance == None:
        ...
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        gainOk, shiftOk, gainVal = ConfigureStreams(engine, handle, streamID, anc_mic_path.stream_gain, anc_mic_path.stream_gain_shift, gainDB, instance_mask)
        if anc_mic_path.name == 'FFA0':
...
def AncSetGainFFA0(engine, handle, gainDB):
    ...
    if not(success):
        ...
        updatePsdata()
        AncSetGain(engine, handle, gainDB, INSTANCE_0_MASK, FFA0)
...
def AncSetGainFFB0(engine, handle, gainDB):
    ...
    if not(success):
        ...
        updatePsdata()
        AncSetGain(engine, handle, gainDB, INSTANCE_0_MASK, FFB0)
...
def AncSetGainFB0(engine, handle, gainDB):
    ...
    if not(success):
        ...
        updatePsdata()
        AncSetGain(engine, handle, gainDB, INSTANCE_0_MASK, FB0)
...
def AncSetGainFFA1(engine, handle, gainDB):
    ...
    if not(success):
        ...
        updatePsdata()
        AncSetGain(engine, handle, gainDB, INSTANCE_1_MASK, FFA1)
...
def AncSetGainFFB1(engine, handle, gainDB):
    ...
    if not(success):
        ...
        updatePsdata()
        AncSetGain(engine, handle, gainDB, INSTANCE_1_MASK, FFB1)
...
def AncSetGainFB1(engine, handle, gainDB):
    ...
    if not(success):
        ...
        updatePsdata()
        AncSetGain(engine, handle, gainDB, INSTANCE_1_MASK, FB1)
...
def LPFSetShiftGain(engine, handle, gainShift, instance_mask, anc_mic_path):
    """
    """
    if anc_mic_path.instance == None:
        print('Connection has not been configured')
        ...
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        shiftOk = audioConfigure(engine, handle, streamID, anc_mic_path.stream_gain_shift, gainShift | instance_mask)
        if not(shiftOk):
            raise ValueError('Error configuring coarse gain, please check that selected audio path is active')
        else:
...
def RxMixSetGain(engine, handle, gainShift, instance_mask, anc_mic_path):
    """
    """
    if anc_mic_path.instance == None:
        print('Connection has not been configured')
        ...
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        gainOk = audioConfigure(engine, handle, streamID, anc_mic_path.rx_mix_gain, gainShift | instance_mask)
        if not(gainOk):
            raise ValueError('Error configuring rxmix gain, please check that selected audio path is active')
        else:
            print(anc_mic_path.name, ': gain changed to ', gainShift)
            if anc_mic_path.name == 'FFA0':
                setParam('OFFSET_ANC_RX_MIX_FF_A_GAIN_L', gainShift)
            elif anc_mic_path.name == 'FFA1':
                setParam('OFFSET_ANC_RX_MIX_FF_A_GAIN_R', gainShift)
            elif anc_mic_path.name == 'FFB0':
                setParam('OFFSET_ANC_RX_MIX_FF_B_GAIN_L', gainShift)
            elif anc_mic_path.name == 'FFB1':
                setParam('OFFSET_ANC_RX_MIX_FF_B_GAIN_R', gainShift)
    return gainOk

def RxMixSetShift(engine, handle, shift, instance_mask, anc_mic_path):
    """
    """
    if anc_mic_path.instance == None:
        print('Connection has not been configured')
        ...
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        shiftOk = audioConfigure(engine, handle, streamID, anc_mic_path.rx_mix_shift, shift | instance_mask)
        if not(shiftOk):
            raise ValueError('Error configuring rxmix shift, please check that selected audio path is active')
        else:
            print(anc_mic_path.name, ': Shift gain changed to ', shift)
            if anc_mic_path.name == 'FFA0':
                setParam('OFFSET_ANC_RX_MIX_FFa_SHIFT_L', shift)
            elif anc_mic_path.name == 'FFA1':
                setParam('OFFSET_ANC_RX_MIX_FFa_SHIFT_R', shift)
            elif anc_mic_path.name == 'FFB0':
                setParam('OFFSET_ANC_RX_MIX_FFb_SHIFT_L', shift)
            elif anc_mic_path.name == 'FFB1':
                setParam('OFFSET_ANC_RX_MIX_FFb_SHIFT_R', shift)
    return shiftOk

def LPFSetFineGain(engine, handle, gainFine, instance_mask, anc_mic_path):
    if anc_mic_path.instance == None:
        print('Connection has not been configured')
        gainOk =  0
    else:
        streamID = getSourceID(engine, handle, anc_mic_path.device, anc_mic_path.instance, anc_mic_path.channel)
        gainOk = audioConfigure(engine, handle, streamID, anc_mic_path.stream_gain, gainFine | instance_mask)
        if not(gainOk):
            raise ValueError('Error configuring fine gain, please check that selected audio path is active')
        else:
...
def SetIIR_Coeffs_FFA0(engine, handle, coeffVec_float):
    # Coefficients are written to chip in 12-bit format or 32-bit format (based on board)
    coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS)
    coeffVec = coeffVec[0]
    mul = 1

    if(len(coeffVec) != IIR_NUM_COEFFS):
        raise ValueError('Number of coefficients mismatch')
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_convertTo16(coeffVec)
        mul = 2
    __coefficients = (ct.c_uint16 * len(coeffVec))(*coeffVec)
    success = engine.teAudioSetAncIirFilter(handle, ct.c_uint16(ANC_INSTANCE_ANC0_ID), ct.c_uint16(ANC_PATH_FFA_ID), __coefficients, ct.c_uint16(IIR_NUM_COEFFS * mul))
    # Set coefficients in the local data structure in 16-bit format for human-readable text file
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS) #TODO: For mora 2.0, change 19 as human READABLE format in ANC_CONSTANTS. For mora2.0, Q7.24 (same as after line 1049)
    elif(BOARD == ANC_V1P9):
        coeffVec = coeffs_float2hex(coeffVec_float, 32, 19)
    coeffVec = iter(coeffVec[0])
    setParam('OFFSET_ANC_FF_A_DEN_COEFF0_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF1_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF2_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF3_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF4_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF5_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF6_L', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_A_DEN_COEFF7_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF0_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF1_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF2_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF3_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF4_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF5_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF6_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF7_L', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_A_NUM_COEFF8_L', next(coeffVec))
...
def SetIIR_Coeffs_FFA1(engine, handle, coeffVec_float):
    ...
    if(BOARD == ANC_V2P0):
        ...
        mul = 2
    __coefficients = (ct.c_uint16 * len(coeffVec))(*coeffVec)
    success = engine.teAudioSetAncIirFilter(handle, ct.c_uint16(ANC_INSTANCE_ANC1_ID), ct.c_uint16(ANC_PATH_FFA_ID), __coefficients, ct.c_uint16(IIR_NUM_COEFFS * mul))
    # Set coefficients in the local data structure in 16-bit format for human-readable text file
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS)
    elif(BOARD == ANC_V1P9):
        coeffVec = coeffs_float2hex(coeffVec_float, 32, 19)
    coeffVec = iter(coeffVec[0])
    setParam('OFFSET_ANC_FF_A_DEN_COEFF0_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF1_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF2_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF3_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF4_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF5_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_DEN_COEFF6_R', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_A_DEN_COEFF7_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF0_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF1_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF2_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF3_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF4_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF5_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF6_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_A_NUM_COEFF7_R', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_A_NUM_COEFF8_R', next(coeffVec))
...
def SetIIR_Coeffs_FFB0(engine, handle, coeffVec_float):
    ...
    if(BOARD == ANC_V2P0):
        ...
        mul = 2
    __coefficients = (ct.c_uint16 * len(coeffVec))(*coeffVec)
    success = engine.teAudioSetAncIirFilter(handle, ct.c_uint16(ANC_INSTANCE_ANC0_ID), ct.c_uint16(ANC_PATH_FFB_ID), __coefficients, ct.c_uint16(IIR_NUM_COEFFS * mul))
    # Set coefficients in the local data structure in 16-bit format for human-readable text file
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS)
    elif(BOARD == ANC_V1P9):
        coeffVec = coeffs_float2hex(coeffVec_float, 32, 19)
    coeffVec = iter(coeffVec[0])
    setParam('OFFSET_ANC_FF_B_DEN_COEFF0_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF1_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF2_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF3_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF4_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF5_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF6_L', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_B_DEN_COEFF7_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF0_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF1_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF2_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF3_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF4_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF5_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF6_L', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF7_L', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_B_NUM_COEFF8_L', next(coeffVec))
...
def SetIIR_Coeffs_FFB1(engine, handle, coeffVec_float):
    ...
    if(BOARD == ANC_V2P0):
        ...
        mul = 2
    __coefficients = (ct.c_uint16 * len(coeffVec))(*coeffVec)
    success = engine.teAudioSetAncIirFilter(handle, ct.c_uint16(ANC_INSTANCE_ANC1_ID), ct.c_uint16(ANC_PATH_FFB_ID), __coefficients, ct.c_uint16(IIR_NUM_COEFFS * mul))
    # Set coefficients in the local data structure in 16-bit format for human-readable text file
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS)
    elif(BOARD == ANC_V1P9):
        coeffVec = coeffs_float2hex(coeffVec_float, 32, 19)
    coeffVec = iter(coeffVec[0])
    setParam('OFFSET_ANC_FF_B_DEN_COEFF0_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF1_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF2_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF3_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF4_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF5_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_DEN_COEFF6_R', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_B_DEN_COEFF7_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF0_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF1_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF2_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF3_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF4_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF5_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF6_R', next(coeffVec))
    setParam('OFFSET_ANC_FF_B_NUM_COEFF7_R', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FF_B_NUM_COEFF8_R', next(coeffVec))
...
def SetIIR_Coeffs_FB0(engine, handle, coeffVec_float):
    ...
    if(BOARD == ANC_V2P0):
        ...
        mul = 2
    __coefficients = (ct.c_uint16 * len(coeffVec))(*coeffVec)
    success = engine.teAudioSetAncIirFilter(handle, ct.c_uint16(ANC_INSTANCE_ANC0_ID), ct.c_uint16(ANC_PATH_FB_ID), __coefficients, ct.c_uint16(IIR_NUM_COEFFS * mul))
    # Set coefficients in the local data structure in 16-bit format for human-readable text file
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS)
    elif(BOARD == ANC_V1P9):
        coeffVec = coeffs_float2hex(coeffVec_float, 32, 19)
    coeffVec = iter(coeffVec[0])
    setParam('OFFSET_ANC_FB_DEN_COEFF0_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF1_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF2_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF3_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF4_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF5_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF6_L', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FB_DEN_COEFF7_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF0_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF1_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF2_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF3_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF4_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF5_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF6_L', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF7_L', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FB_NUM_COEFF8_L', next(coeffVec))
...
def SetIIR_Coeffs_FB1(engine, handle, coeffVec_float):
    ...
    if(BOARD == ANC_V2P0):
        ...
        mul = 2
    __coefficients = (ct.c_uint16 * len(coeffVec))(*coeffVec)
    success = engine.teAudioSetAncIirFilter(handle, ct.c_uint16(ANC_INSTANCE_ANC1_ID), ct.c_uint16(ANC_PATH_FB_ID), __coefficients, ct.c_uint16(IIR_NUM_COEFFS * mul))
    # Set coefficients in the local data structure in 16-bit format for human-readable text file
    if(BOARD == ANC_V2P0):
        coeffVec = coeffs_float2hex(coeffVec_float, IIR_NUM_BITS, IIR_INT_BITS)
    elif(BOARD == ANC_V1P9):
        coeffVec = coeffs_float2hex(coeffVec_float, 32, 19)
    coeffVec = iter(coeffVec[0])
    setParam('OFFSET_ANC_FB_DEN_COEFF0_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF1_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF2_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF3_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF4_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF5_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_DEN_COEFF6_R', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FB_DEN_COEFF7_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF0_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF1_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF2_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF3_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF4_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF5_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF6_R', next(coeffVec))
    setParam('OFFSET_ANC_FB_NUM_COEFF7_R', next(coeffVec))
    if(BOARD == ANC_V2P0):
        setParam('OFFSET_ANC_FB_NUM_COEFF8_R', next(coeffVec))
...
def coeffs_float2hex(coeffs, Nbits, Q_num):
    np.asarray(coeffs)
    regSize = Nbits;
    NegShift = 2**regSize
...
def coeffs_convertTo16(coeffs):
    num_bits = 16
    mask = 0xffff
    coeffs_16 = []
    for coe in coeffs:
        coeffs_16.append(coe & mask)
        coeffs_16.append(coe >> num_bits)
    return coeffs_16
#################################################
...
def EnableDCFilter(engine, handle, enableFlag):
    ...
    """
    dcFilter_Enable(engine, handle, FFA0, enableFlag, INSTANCE_0_MASK)
    dcFilter_Enable(engine, handle, FFB0, enableFlag, INSTANCE_0_MASK)
    dcFilter_Enable(engine, handle, FFA1, enableFlag, INSTANCE_1_MASK)
    dcFilter_Enable(engine, handle, FFB1, enableFlag, INSTANCE_1_MASK)
...
def EnableFBMon(engine, handle, selectFlag):
    """
    # Enables FBMon for both ANC instances
    """
    fbmon_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, selectFlag)
    fbmon_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, selectFlag)

def EnableFBMon_Left(engine, handle, selectFlag):
    """
    # Enables FB Mon for all ANC0 paths
    """
    fbmon_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, selectFlag)

def EnableFBMon_Right(engine, handle, selectFlag):
    """
    # Enables FB Mon for all ANC1 paths
    """
    fbmon_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, selectFlag)

def EnableX2ASelect(engine, handle, enableFlag):
    """
    # Enables X2 A for both ANC instances
    """
    x2_a_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
    x2_a_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX2A_Left(engine, handle, enableFlag):
    """
    # Enables X2 A for all ANC0 paths
    """
    x2_a_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableX2A_Right(engine, handle, enableFlag):
    """
    # Enables X2 A for all ANC1 paths
    """
    x2_a_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX2BSelect(engine, handle, enableFlag):
    """
    # Enables X2 B for both ANC instances
    """
    x2_b_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
    x2_b_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX2B_Left(engine, handle, enableFlag):
    """
    # Enables X2 B for all ANC0 paths
    """
    x2_b_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableX2B_Right(engine, handle, enableFlag):
    """
    # Enables X2 B for all ANC1 paths
    """
    x2_b_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX0P5ASelect(engine, handle, enableFlag):
    """
    # Enables X0P5 A for both ANC instances
    """
    x0p5_a_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
    x0p5_a_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX0P5A_Left(engine, handle, enableFlag):
    """
    # Enables X0P5 A for all ANC0 paths
    """
    x0p5_a_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableX0P5A_Right(engine, handle, enableFlag):
    """
    # Enables X0P5 A for all ANC1 paths
    """
    x0p5_a_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX0P5BSelect(engine, handle, enableFlag):
    """
    # Enables X0P5 B for both ANC instances
    """
    x0p5_b_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
    x0p5_b_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableX0P5B_Left(engine, handle, enableFlag):
    """
    # Enables X0P5 B for all ANC0 paths
    """
    x0p5_b_select(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableX0P5B_Right(engine, handle, enableFlag):
    """
    # Enables X0P5 B for all ANC1 paths
    """
    x0p5_b_select(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableSelfRxMix(engine, handle, enableFlag):
    """
    # Enables Self RxMix for both ANC instances
    """
    self_rxmix_Enable(engine, handle, Anc0Control1, ANC_INSTANCE_ANC0_ID, enableFlag)
    self_rxmix_Enable(engine, handle, Anc1Control1, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableSelfRxMix_Left(engine, handle, enableFlag):
    """
    # Enables Self RxMix for all ANC0 paths
    """
    self_rxmix_Enable(engine, handle, Anc0Control1, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableSelfRxMix_Right(engine, handle, enableFlag):
    """
    # Enables Self RxMix for all ANC1 paths
    """
    self_rxmix_Enable(engine, handle, Anc1Control1, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableCrossRxMix(engine, handle, enableFlag):
    """
    # Enables Cross RxMix for both ANC instances
    """
    cross_rxmix_Enable(engine, handle, Anc0Control1, ANC_INSTANCE_ANC0_ID, enableFlag)
    cross_rxmix_Enable(engine, handle, Anc1Control1, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableCrossRxMix_Left(engine, handle, enableFlag):
    """
    # Enables Cross RxMix for all ANC0 paths
    """
    cross_rxmix_Enable(engine, handle, Anc0Control1, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableCrossRxMix_Right(engine, handle, enableFlag):
    """
    # Enables Cross RxMix for all ANC1 paths
    """
    cross_rxmix_Enable(engine, handle, Anc1Control1, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableFFLE(engine, handle, enableFlag):
    """
    # Enables FFLE for both ANC instances
    """
    if(BOARD == ANC_V1P9):
        ffle_Enable(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
        ffle_Enable(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)
    elif(BOARD == ANC_V2P0):
        ffle_Enable(engine, handle, Anc0Control1, ANC_INSTANCE_ANC0_ID, enableFlag)
        ffle_Enable(engine, handle, Anc1Control1, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableFFLE_Left(engine, handle, enableFlag):
    """
    # Enables FFLE for all ANC0 paths
    """
    if(BOARD == ANC_V1P9):
        ffle_Enable(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
    elif(BOARD == ANC_V2P0):
...
def EnableFFLE_Right(engine, handle, enableFlag):
    """
    # Enables FFLE for all ANC1 paths
    """
    if(BOARD == ANC_V1P9):
        ffle_Enable(engine, handle, Anc1Control1, ANC_INSTANCE_ANC1_ID, enableFlag)
    elif(BOARD == ANC_V2P0):
...
def EnableFFGainZCD(engine, handle, enableFlag):
    """
    # Enables FF Gain ZCD for both ANC instances
    """
    ffgain_zcd_Enable(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)
    ffgain_zcd_Enable(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableFFGainZCD_Left(engine, handle, enableFlag):
    """
    # Enables FF Gain ZCD for all ANC0 paths
    """
    ffgain_zcd_Enable(engine, handle, Anc0Control, ANC_INSTANCE_ANC0_ID, enableFlag)

def EnableFFGainZCD_Right(engine, handle, enableFlag):
    """
    # Enables FFGain ZCD for all ANC1 paths
    """
    ffgain_zcd_Enable(engine, handle, Anc1Control, ANC_INSTANCE_ANC1_ID, enableFlag)

def EnableOutmix(engine, handle, enableFlag):
    """
    # Enables Outmix for ANC0
    """
    outmix_Enable(engine, handle, Anc0Control, enableFlag)

def EnableAnc1usesAnc0(engine, handle, enableFlag):
    """
    # Enables ANC1_USES_ANC0_RX_PCM_INPUT
    """
    anc1usesAnc0_Enable(engine, handle, Anc1Control1, enableFlag)

def EnableSMLPF(engine, handle, enableFlag):
    """
    # Enables SMLPF block for all paths (ANC0, ANC1)
    """
    smLPFEnable(engine, handle, FFA0, enableFlag, INSTANCE_0_MASK)
    smLPFEnable(engine, handle, FFA1, enableFlag, INSTANCE_1_MASK)
...
def SetDCFilter_Shift(engine, handle, shift):
    ...
    """
    dcFilter_Shift(engine, handle, FFA0, shift, INSTANCE_0_MASK)
    dcFilter_Shift(engine, handle, FFB0, shift, INSTANCE_0_MASK)
    dcFilter_Shift(engine, handle, FFA1, shift, INSTANCE_1_MASK)
    dcFilter_Shift(engine, handle, FFB1, shift, INSTANCE_1_MASK)
...
def SetSMLPF_Shift(engine, handle, shift):
    ...
    """
    smLPFShift(engine, handle, FFA0, shift, INSTANCE_0_MASK)
    smLPFShift(engine, handle, FFA1, shift, INSTANCE_1_MASK)
...
def SetGain_Shift_FFa0(engine, handle, gainShift):
    ...
    """
    LPFSetShiftGain(engine, handle, gainShift, INSTANCE_0_MASK, FFA0)

def SetGain_Shift_FFb0(engine, handle, gainShift):
    """
    # Set shift value for FFb0
    """
    LPFSetShiftGain(engine, handle, gainShift, INSTANCE_0_MASK, FFB0)

def SetGain_Shift_FB0(engine, handle, gainShift):
    """
    # Set shift value for FB0
    """
    LPFSetShiftGain(engine, handle, gainShift, INSTANCE_0_MASK, FB0)

def SetGain_Shift_FFa1(engine, handle, gainShift):
    """
    # Set shift value for FFa1
    """
    LPFSetShiftGain(engine, handle, gainShift, INSTANCE_1_MASK, FFA1)

def SetGain_Shift_FFb1(engine, handle, gainShift):
    """
    # Set shift value for FFb1
    """
    LPFSetShiftGain(engine, handle, gainShift, INSTANCE_1_MASK, FFB1)

def SetGain_Shift_FB1(engine, handle, gainShift):
    """
    # Set shift value for FB1
    """
    LPFSetShiftGain(engine, handle, gainShift, INSTANCE_1_MASK, FB1)
...
def RxMix_Gain_FFa0(engine, handle, gainShift):
    """
    # Set gain value for FFa0
    """
    RxMixSetGain(engine, handle, gainShift, INSTANCE_0_MASK, FFA0)

def RxMix_Gain_FFb0(engine, handle, gainShift):
    """
    # Set gain value for FFb0
    """
    RxMixSetGain(engine, handle, gainShift, INSTANCE_0_MASK, FFB0)

def RxMix_Gain_FFa1(engine, handle, gainShift):
    """
    # Set gain value for FFa1
    """
    RxMixSetGain(engine, handle, gainShift, INSTANCE_1_MASK, FFA1)

def RxMix_Gain_FFb1(engine, handle, gainShift):
    """
    # Set gain value for FFb1
    """
    RxMixSetGain(engine, handle, gainShift, INSTANCE_1_MASK, FFB1)

def RxMix_Gain_FFaLeft(engine, handle, gainShift):
    """
    # Set gain value for FFa0
    """
    RxMix_Gain_FFa0(engine, handle, gainShift)

def RxMix_Gain_FFbLeft(engine, handle, gainShift):
    """
    # Set shift value for FFb0
    """
    RxMix_Gain_FFb0(engine, handle, gainShift)

def RxMix_Gain_FFaRight(engine, handle, gainShift):
    """
    # Set shift value for FFa1
    """
    RxMix_Gain_FFa1(engine, handle, gainShift)

def RxMix_Gain_FFbRight(engine, handle, gainShift):
    """
    # Set shift value for FFb1
    """
    RxMix_Gain_FFb1(engine, handle, gainShift)

def RxMix_Shift_FFa0(engine, handle, shift):
    """
    # Set shift value for FFa0
    """
    RxMixSetShift(engine, handle, shift, INSTANCE_0_MASK, FFA0)

def RxMix_Shift_FFb0(engine, handle, shift):
    """
    # Set shift value for FFb0
    """
    RxMixSetShift(engine, handle, shift, INSTANCE_0_MASK, FFB0)

def RxMix_Shift_FFa1(engine, handle, shift):
    """
    # Set shift value for FFa1
    """
    RxMixSetShift(engine, handle, shift, INSTANCE_1_MASK, FFA1)

def RxMix_Shift_FFb1(engine, handle, shift):
    """
    # Set shift value for FFb1
    """
    RxMixSetShift(engine, handle, shift, INSTANCE_1_MASK, FFB1)

def RxMix_Shift_FFaLeft(engine, handle, shift):
    """
    # Set shift value for FFa0
    """
    RxMix_Shift_FFa0(engine, handle, shift)

def RxMix_Shift_FFbLeft(engine, handle, shift):
    """
    # Set shift value for FFb0
    """
    RxMix_Shift_FFb0(engine, handle, shift)

def RxMix_Shift_FFaRight(engine, handle, shift):
    """
    # Set shift value for FFa1
    """
    RxMix_Shift_FFa1(engine, handle, shift)

def RxMix_Shift_FFbRight(engine, handle, shift):
    """
    # Set shift value for FFb1
    """
    RxMix_Shift_FFb1(engine, handle, shift)
'''
###################
################### Gain (Fine) functions ################
###################
...
def SetGain_Fine_FFa0(engine, handle, gainFine):
    ...
    """
    LPFSetFineGain(engine, handle, gainFine, INSTANCE_0_MASK, FFA0)

def SetGain_Fine_FFb0(engine, handle, gainFine):
    """
    # Set fine value for FFb0
    """
    LPFSetFineGain(engine, handle, gainFine, INSTANCE_0_MASK, FFB0)

def SetGain_Fine_FB0(engine, handle, gainFine):
    """
    # Set fine value for FB0
    """
    LPFSetFineGain(engine, handle, gainFine, INSTANCE_0_MASK, FB0)

def SetGain_Fine_FFa1(engine, handle, gainFine):
    """
    # Set fine value for FFa1
    """
    LPFSetFineGain(engine, handle, gainFine, INSTANCE_1_MASK, FFA1)

def SetGain_Fine_FFb1(engine, handle, gainFine):
    """
    # Set fine value for FFb1
    """
    LPFSetFineGain(engine, handle, gainFine, INSTANCE_1_MASK, FFB1)

def SetGain_Fine_FB1(engine, handle, gainFine):
    """
    # Set fine value for FB1
    """
    LPFSetFineGain(engine, handle, gainFine, INSTANCE_1_MASK, FB1)
...
def pullANCparams(engine, handle, filename, filter_number):
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  DEN_COEFF7  : 0x%08X' % (getParam('OFFSET_ANC_FF_B_DEN_COEFF7_L')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  NUM_COEFF8  : 0x%08X' % (getParam('OFFSET_ANC_FF_B_NUM_COEFF8_L')))
        registerList.append('RxMix')
        registerList.append('  RxMix Gain : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FF_B_GAIN_L')))
        registerList.append('  RxMix Shift : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FFb_SHIFT_L')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  DEN_COEFF7  : 0x%08X' % (getParam('OFFSET_ANC_FF_A_DEN_COEFF7_L')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  NUM_COEFF8  : 0x%08X' % (getParam('OFFSET_ANC_FF_A_NUM_COEFF8_L')))
        registerList.append('RxMix')
        registerList.append('  RxMix Gain : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FF_A_GAIN_L')))
        registerList.append('  RxMix Shift : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FFa_SHIFT_L')))
    registerList.append('')
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  DEN_COEFF7  : 0x%08X' % (getParam('OFFSET_ANC_FB_DEN_COEFF7_L')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  NUM_COEFF8  : 0x%08X' % (getParam('OFFSET_ANC_FB_NUM_COEFF8_L')))
    registerList.append('')
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  DEN_COEFF7  : 0x%08X' % (getParam('OFFSET_ANC_FF_B_DEN_COEFF7_R')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  NUM_COEFF8  : 0x%08X' % (getParam('OFFSET_ANC_FF_B_NUM_COEFF8_R')))
        registerList.append('RxMix')
        registerList.append('  RxMix Gain : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FF_B_GAIN_R')))
        registerList.append('  RxMix Shift : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FFb_SHIFT_R')))
    registerList.append('')
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  DEN_COEFF7  : 0x%08X' % (getParam('OFFSET_ANC_FF_A_DEN_COEFF7_R')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  NUM_COEFF8  : 0x%08X' % (getParam('OFFSET_ANC_FF_A_NUM_COEFF8_R')))
        registerList.append('RxMix')
        registerList.append('  RxMix Gain : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FF_A_GAIN_R')))
        registerList.append('  RxMix Shift : 0x%08X' % (getParam('OFFSET_ANC_RX_MIX_FFa_SHIFT_R')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  DEN_COEFF7  : 0x%08X' % (getParam('OFFSET_ANC_FB_DEN_COEFF7_R')))
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  NUM_COEFF8  : 0x%08X' % (getParam('OFFSET_ANC_FB_NUM_COEFF8_R')))
    # Including the remainder of the ANC_PARAM_LIST variables that are populated in psdata and written to HTF
    registerList.append('')
    registerList.append('OTHER_ANC_PARAMETERS_Left')
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  OFFSET_ANC_OUTMIX_ENABLE_L    : 0x%08X' % (getParam('OFFSET_ANC_OUTMIX_ENABLE_L')))
        registerList.append('  OFFSET_SELF_RXMIX_ENABLE_L    : 0x%08X' % (getParam('OFFSET_SELF_RXMIX_ENABLE_L')))
        registerList.append('  OFFSET_CROSS_RXMIX_ENABLE_L    : 0x%08X' % (getParam('OFFSET_CROSS_RXMIX_ENABLE_L')))
        registerList.append('  OFFSET_ANC0_USES_ANC1_RX_PCM_INPUT    : 0x%08X' % (getParam('OFFSET_ANC0_USES_ANC1_RX_PCM_INPUT')))

    registerList.append('')
    registerList.append('OTHER_ANC_PARAMETERS_Right')
    ...
    if(BOARD == ANC_V2P0):
        registerList.append('  OFFSET_SELF_RXMIX_ENABLE_R    : 0x%08X' % (getParam('OFFSET_SELF_RXMIX_ENABLE_R')))
        registerList.append('  OFFSET_CROSS_RXMIX_ENABLE_R    : 0x%08X' % (getParam('OFFSET_CROSS_RXMIX_ENABLE_R')))
        registerList.append('  OFFSET_ANC1_USES_ANC0_RX_PCM_INPUT    : 0x%08X' % (getParam('OFFSET_ANC1_USES_ANC0_RX_PCM_INPUT')))
...
def pushANCparams(engine, handle, filename, filter_number):
    ...
    for line in fileIn:
        ...
        if 'LEFT_FEEDFORWARD_(FF)_FILTER_BLOCK_PARAMETERS' in line:
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_B_DEN_COEFF7_L', read_line_from_file())
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_B_NUM_COEFF8_L', read_line_from_file())
                next(fileIn, '')
                setParam('OFFSET_ANC_RX_MIX_FF_B_GAIN_L', read_line_from_file())
                setParam('OFFSET_ANC_RX_MIX_FFb_SHIFT_L', read_line_from_file())
        ...
        if 'LEFT_FEEDBACK_(FB)_FILTER_BLOCK_PARAMETERS' in line:
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_A_DEN_COEFF7_L', read_line_from_file())
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_A_NUM_COEFF8_L', read_line_from_file())
                next(fileIn, '')
                setParam('OFFSET_ANC_RX_MIX_FF_A_GAIN_L', read_line_from_file())
                setParam('OFFSET_ANC_RX_MIX_FFa_SHIFT_L', read_line_from_file())
        ...
        if 'LEFT_ECHO_CANCELLATION_(EC)_FILTER_BLOCK_PARAMETERS' in line:
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FB_DEN_COEFF7_L', read_line_from_file())
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FB_NUM_COEFF8_L', read_line_from_file())
        if 'RIGHT_FEEDFORWARD_(FF)_FILTER_BLOCK_PARAMETERS' in line:
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_B_DEN_COEFF7_R', read_line_from_file())
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_B_NUM_COEFF8_R', read_line_from_file())
                next(fileIn, '')
                setParam('OFFSET_ANC_RX_MIX_FF_B_GAIN_R', read_line_from_file())
                setParam('OFFSET_ANC_RX_MIX_FFb_SHIFT_R', read_line_from_file())
        ...
        if 'RIGHT_FEEDBACK_(FB)_FILTER_BLOCK_PARAMETERS' in line:
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_A_DEN_COEFF7_R', read_line_from_file())
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FF_A_NUM_COEFF8_R', read_line_from_file())
                next(fileIn, '')
                setParam('OFFSET_ANC_RX_MIX_FF_A_GAIN_R', read_line_from_file())
                setParam('OFFSET_ANC_RX_MIX_FFa_SHIFT_R', read_line_from_file())
        if 'RIGHT_ECHO_CANCELLATION_(EC)_FILTER_BLOCK_PARAMETERS' in line:
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FB_DEN_COEFF7_R', read_line_from_file())
            ...
            if(BOARD == ANC_V2P0):
                setParam('OFFSET_ANC_FB_NUM_COEFF8_R', read_line_from_file())
    fileIn.close()
    updatePsdata()
